...
| `SLACKBOTV2_CHANNEL_DEFAULTS` | `slackbotv2.channelDefaults`. | Per-channel default model / reasoning effort as a JSON object keyed by Slack conversation id, e.g. `{"C0ENG":{"model":"claude-opus-4-8","reasoning":"high"},"C0TRIAGE":{"reasoning":"low"}}`. Applied when a message in that channel carries no explicit `--model`/`-rsn` override (below such an override, above the deployment/baked default) and forwarded onto the harness input line so it takes effect. Set a model compatible with the deployment's default harness; `reasoning` only affects the codex harness. Malformed JSON is logged and ignored. |
